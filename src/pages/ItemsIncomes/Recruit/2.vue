<template>
  <table id="data_table" class="column12p">
    <thead>
      <tr>
        <th><span class="blank"></span></th>
        <th>ターゲット名</th>
        <th>最終活動日</th>
        <th>活動内容</th>
        <th>年齢</th>
        <th>前職</th>
        <th>応募ルート</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="(date,itemObjectKey) in dates">
        <td><input type="checkbox" name="" value="" class="input_check"></td>
        <td><p class="btn_update"><a href="" @click.prevent="adoptItemClick(itemObjectKey)">更新</a></p><span class="planTitle">朝日産業　株式会社</span></td>
        <td></td>
        <td></td>
        <td><input type="text" class="input_text" value=""></td>
        <td><input type="text" class="input_text" value=""></td>
        <td>
          <select name="" class="input_select">
            <option value="">紹介（代理店）</option>
            <option value="">紹介（研修生）</option>
            <option value="">紹介（NLI）</option>
            <option value="">ハローワーク</option>
            <option value="">スカウト</option>
            <option value="">提案</option>
            <option value="">その他</option>
          </select>
        </td>
      </tr>
    </tbody>
  </table><!--/data_table-->
</template>

<script>
import { mapGetters, mapActions } from 'vuex'
import $ from '../../.././assets/jquery-1.12.4.min'

export default {
  data () {
    return {
      dates: window.dates.adopt[1]
    }
  },
  mounted () {
  },
  computed: {
    ...mapGetters({
      'type': 'auth/type'
    })
  },
  methods: {
    ...mapActions({
      togglePopupVisible: 'popup/togglePopupVisible'
    }),
    updateStatus () {
      console.log('Success')
    },
    adoptItemClick (index) {
      $('#data_table tbody tr:eq(' + index + ')').addClass('popOn')
      this.togglePopupVisible('target')
    }
  }
}
</script>